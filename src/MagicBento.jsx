...
const BentoCardGrid = ({ children, gridRef, layoutStyle = 'default' }) => (
  <div className={`card-grid bento-section ${layoutStyle === 'asymmetric' ? 'asymmetric-layout' : ''}`} ref={gridRef}>
    {children}
  </div>
);
...
const useMobileScrollAnimation = (gridRef, isMobile) => {
  useEffect(() => {
    // DISABLED - No animations on mobile for case studies
    return;
  }, [gridRef, isMobile]);
};
...
const MagicBento = ({
  ...
  useMobileScrollAnimation(gridRef, isMobile);
  ...
  return (
    <>
      ...
      <BentoCardGrid gridRef={gridRef} layoutStyle={layoutStyle}>
        {cards.map((card, index) => {
          ...
          const isExpanded = expandable && card.id && expandedCases[card.id];

          // Render expandable card (both mobile and desktop if expandable is true)
          if (expandable && card.id) {
            return (
              <div key={index} {...cardProps}>
                {/* Collapsed View */}
                <div className={isExpanded ? 'hidden' : 'block'}>
                  <div className="magic-bento-card__header">
                    {card.icon && <div className="magic-bento-card__icon mb-2">{card.icon}</div>}
                  </div>
